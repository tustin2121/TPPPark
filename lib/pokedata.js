	...
	Pokemon.fn = Pokemon.prototype = new Event({
		...
		openSummaryScreen : function(){
			...
			
			{
				var sources = "";
				if (this.dexsource) {
					if (/^http:\/\//.test(this.dexsource)) {
						sources += "<a href='"+this.dexsource+"'>Pokedex Image</a><br/>";
					}
				}
				if (this.spritesource) {
					if (/^http:\/\//.test(this.spritesource)) {
						sources += "<a href='"+this.spritesource+"'>Sprite Source</a><br/>";
					}
				}
