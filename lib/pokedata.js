...
(function(){
	...
	Event.fn = Event.prototype = {
		...
		infodex_entry : null, //If this item has an entry for the infodex that 
			//is beyond the dedicated classes: Pokemon, Trainer, and TPPer
			...
			var img = $("<img>").attr("src", this.sprite).addClass("main")
				...
				.on("vclick", function(){
					...
					if (!isDragging) {
						...
						if (eventobj.infodex_entry)
							window.infodex.displayExtraEntry(eventobj.infodex_entry);
					}
				});
...
(function(){
	...
	function MondayMassacre_Ribbon() {
		if (!(this instanceof MondayMassacre_Ribbon))
			return new MondayMassacre_Ribbon();
		
		Ribbon.call(this, {});
	}
	MondayMassacre_Ribbon.fn = MondayMassacre_Ribbon.prototype = new Ribbon({
		cssclass : "rb-massmonday",
		name : "Monday Massacre Victim",
		desc : "Always Remember",
	});
	window.MondayMassacre_Ribbon = MondayMassacre_Ribbon;
...
(function(){
	...
	Badge.fn = Badge.prototype = {
		//Badges are dependent on the trainer they're in, to provide the image info
		name: "<Badge>",
		leader: "<Leader>",
		type: "<Type>",
		desc: "<Desc>",
	...
	function Icon(img, desc){
		if (!(this instanceof Icon))
			return new Icon(img, desc);
	...
	Icon.fn = Icon.prototype = {
		img: "",
		desc: "<Desc>",
	};
	window.Icon = Icon;
	
	function InfodexEntry(opts){
		if (!(this instanceof InfodexEntry))
			return new InfodexEntry(opts);
		
		if ($.isPlainObject(opts))
			$.extend(this, opts);
		else
			$.extend(this, {title: opts});
	}
	InfodexEntry.fn = InfodexEntry.prototype = {
		gem : "yellow",
		title: "Extra Info",
		tabs: null,
		sources: null,
		html: "&lt;Error: No information&gt;",
		
		addTab : function(title, html) {
			if (!this.tabs) this.tabs = {};
			this.tabs[title] = html;
		},
	};
	window.InfodexEntry = InfodexEntry;
