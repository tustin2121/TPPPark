...
function addEvent(obj) {
	...
	if (eventList) { //For early on, when the event list is collecting things
		...
		if (getCurrentOccasion) { //if the occasions file is installed
...
function showTimer(str, date) {
	$(function(){
		// $.getScript("lib/jquery.countdown.min.js", function(){
		// 	$(".ui.countdown").countdown({
		// 		until: date, timezone: -4,
		// 		layout: str+"{dn}d {hnn}:{mnn}:{sn}",
		// 	});
		// });
